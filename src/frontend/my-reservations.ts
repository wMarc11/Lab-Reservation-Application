...
/*
  ...
  function formatReservationTime(dateTimeValue: string | Date) {
  ...
  function formatReservationTimeRange(reservation: ReservationDTO) {
  ...
  function renderTableRows(list: ReservationWithStatus[]) {
    ...
    els.tbody.innerHTML = list
      .map((reservation) => {
        ...
        const isCancelledStudentReservation =
          !isManager && reservation._status === "CANCELLED";
  ...
  function generateTimeSlotsForEdit(selectedDateInput: Date) {
    const select = queryElement<HTMLSelectElement>("#edit-start");
    if (!select) return;
    ...
    for (let h = startHour; h < endHour; h++) {
      for (let m of [0, 30]) {
        ...
        option.textContent = `${pad2(slotStart.getHours())}:${pad2(slotStart.getMinutes())} - ${pad2(
          slotEnd.getHours()
        )}:${pad2(slotEnd.getMinutes())}`;
