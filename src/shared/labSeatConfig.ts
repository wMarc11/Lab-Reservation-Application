import { LAB_NAMES, LabName } from "./labNames.js";

export type BuildingCode = "GH" | "STRC" | "SJ" | "LS" | "MH" | "VH";

export type LabLayoutElement =
    | {
          type: "table";
...
export type LabLayoutRow = {
    className?: string;
    items: LabLayoutElement[];
};

export type LabSeatConfig = {
    capacity: number;
...
function inferLabLocation(labName: LabName): { buildingCode: BuildingCode; floor: number } {
...
function createGenericLayout(capacity: number): LabLayoutRow[] {
    const rows: LabLayoutRow[] = [];
    const seatsPerRow = 5;

    for (let seatNumber = 1; seatNumber <= capacity; seatNumber += seatsPerRow) {
        const rowSeatNumbers: number[] = [];
...
export const LAB_SEAT_CONFIG: Record<LabName, LabSeatConfig> = LAB_NAMES.reduce(
    ...
    {} as Record<LabName, LabSeatConfig>
);

export function getLabsForBuildingFloor(buildingCode: BuildingCode, floor: number) {
    return LAB_NAMES.filter((labName) => {
        const config = LAB_SEAT_CONFIG[labName];
        return config.buildingCode === buildingCode && config.floor === floor;
    });
}

export function getLabLayout(room: string) {
    return LAB_SEAT_CONFIG[room as keyof typeof LAB_SEAT_CONFIG]?.layout ?? createGenericLayout(DEFAULT_LAB_CAPACITY);
}

export function getLabCapacity(room: string) {
    return LAB_SEAT_CONFIG[room as keyof typeof LAB_SEAT_CONFIG]?.capacity ?? DEFAULT_LAB_CAPACITY;
}

export function normalizeBuildingCode(value: string | null | undefined): BuildingCode | null {
    if (!value) {
        return null;
    }
...
export function parseFloorNumber(value: string | number | null | undefined): number | null {
